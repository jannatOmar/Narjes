...
class analysisController extends Controller
{
    public function index(){
        ...
        return view('manager.analysis.viewAnalysis', compact("analysis"));
    }
      ...
      try{
        ...
        //   return var_dump($data);
         $countResult =Analysis_requierd::select("analysis_id",DB::raw("COUNT(analysis_id) as count_chick"))
          ...
          $i=0;
          $l=0;
        return view('manager.analysis.viewForm',compact('analysis','input','data_option','i','l','normal_range','data','count_chick'));
      }catch(\Exception $ex){
        return redirect()->back()->with(['error'=>'هناك خطأ ما يرجى اعادة المحاولة']);
      }
     }
     public function createForm(){
        return view('manager.analysis.createNewForm');
     }
     public function AddNewInputs($analysis_id){
       try{
          $analysis=Analysis::where('analysis_id',$analysis_id)->get();
          return view('manager.analysis.addNewInputs',compact('analysis'));
        }catch(\Exception $ex){
        return redirect()->back()->with(['error'=>'هناك خطأ ما يرجى اعادة المحاولة']);
        }
   }
   public function storeNewInputs($analysis_id,addNewInputsRequest $request){
       try{
           ...
           DB::commit();
           return redirect()->route('manager.showAnalysis')->with(['success'=>'تم الحفظ بنجاح']);
 ...
 public function analysisUpdateForm(UpdateFormRequest $request,$analysis_id){
     try{
         ...
         DB::commit();
         return redirect()->route('manager.showAnalysis')->with(['success'=>'تم التعديل بنجاح']);
     }catch(\Exception $ex){
         DB::rollback();
