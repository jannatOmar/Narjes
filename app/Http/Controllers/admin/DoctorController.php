...
class DoctorController extends Controller
{
    ...
    public function index(){
        try {
            ...
            return view('admin.doctor.doctorManagement', compact("doctor"));
        }
        catch (\Exception $ex ){
    ...
    public function storeDoctor(addNewDoctorRequest $request){
        ...
        try{
          ...
          $mess=Auth::user()->username . " add a new doctor: " . $doctor_name;
            ...
            return redirect()->route('admin.doctor.managment')->with(['success'=>'تم الحفظ بنجاح']);
        }catch(\Exception $ex){
            return redirect()->back()->with(['error'=>'هناك خطأ ما يرجى اعادة المحاولة']);
        }
    }
    ...
    public function search(Request $request){
        try{
            ...
            return view('admin.doctor.doctorManagement', compact("doctor","q"));
        }catch(\Exception $ex){
    ...
    public function edit($doctor_id){
        try{
            ...
            if(!$doctor){
                return redirect()->route('admin.doctor.doctorManagement')->with(['error'=>'هذاالمريض غير موجود']);
            }
            return view('admin.doctor.updateDoctor',compact('doctor'));
        }catch(\Exception $ex){
    ...
    public function update(updateDoctorRequest $request, $doctor_id){
        ...
        try{
          ...
          Doctor::where('doctor_id', $doctor_id)
            ...
            $mess = Auth::user()->username . " update a doctor:". $doctor_name ." from [ ".$doctor[0]->f_name. ' , '
            ...
            return redirect()->route('admin.doctor.managment')->with(['success'=>'تم التعديل بنجاح']);
        }catch(\Exception $ex){
